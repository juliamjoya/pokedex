...
class Pokemon {
    constructor(id, name, type, weight, abilities, ThumbnailImage) {
        this.id = id;
        this.name = name;
        this.type = type;
        this.weight = weight;
...
fetch('pokemones.json')
    ...
    .then(data => {
        for (let pokemonData of data) {
            let pokemon = new Pokemon(pokemonData.id, pokemonData.name, pokemonData.type, pokemonData.weight, pokemonData.abilities, pokemonData.ThumbnailImage);
            listaPokemon.push(pokemon);
            //console.log(pokemonData.ThumbnailImage);
        }
        mostrarListaPokemon();
    });
